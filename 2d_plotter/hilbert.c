...
static inline void draw_line_dir(Axes2D * pos, Direction d, int l) {
    draw_line_to(pos, pos->X.pos + MSIN(d) * l, pos->Y.pos + MSIN((d + 1) % END) * l);
}

// Hilbert curve recursive algorithm as L-system (https://en.wikipedia.org/wiki/Hilbert_curve)

void draw_hilbert_rec(Axes2D * pos, int level, Direction direction, int length) {
    if (!--level) {
